...
int main(){
    ...
    while(t--){
        ...
        map<int, bool> mp; //khai báo map để lưu key của mảng b
        //duyệt + nhập các phần tử (key) vào mảng b và lưu vào map
        for(int i = 0; i < n; i++){
            int x; cin >> x;
            mp[x] = true; //cho các value của map của mảng b = 1 vì không sử dụng
        }
        bool oke = false;//khởi tạo biến oke để check xem phần tử có xuất hiện hay không
        //duyệt các phần tử của mảng a
        for(int i : a){
            //tìm xem trong mảng b(map) có các phần tử key là i của mảng a hay không
            //nếu trong dãy b không tồn tại phần tử key là i đó thì in ra
            if(mp.find(i) == mp.end()){
                cout << i <<" ";
